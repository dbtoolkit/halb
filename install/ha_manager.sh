...
vrrp_script vs_${O_P_DBTYPE}_${O_P_VIRTUAL_ROUTER_ID} {
    script "/etc/keepalived/check_mysql.pl --vrrp-instance=${O_P_VRRPINSTANCE} --total-timeout-seconds=60"
    interval 60
}
...
##end pxcr
    ...
    if [ $_vrrp_num -eq 0 ]; then
      ...
      cat >>$_tf <<EOF
vrrp_script vs_${O_P_DBTYPE}_${O_P_VIRTUAL_ROUTER_ID} {
    script "/etc/keepalived/check_lvs.sh ${O_P_VRRPINSTANCE}"
    interval 60
}
vrrp_instance vi_${O_P_DBTYPE}_${O_P_VIRTUAL_ROUTER_ID} {
    state BACKUP
...
virtual_server ${O_P_VIP} ${O_P_SIDORPORT} {
    delay_loop 15
    lb_algo wlc
    lb_kind DR
    protocol TCP
    ...
    real_server ${O_P_DEST_IP_ARRAY[0]} ${O_P_SIDORPORT} {
        ...
        inhibit_on_failure
        MISC_CHECK {
            misc_path "/etc/keepalived/check_realserver.sh ${O_P_DEST_IP_ARRAY[$_i]} ${O_P_SIDORPORT}"
            misc_timeout 15
            misc_dynamic
        }
    }
EOF
      _r=$?
#    for((_i=1;_i<${#O_P_DEST_IP_ARRAY[@]};_i++)); do
#      cat >>$_tf <<EOF
#    real_server ${O_P_DEST_IP_ARRAY[$_i]} ${O_P_SIDORPORT} {
#        weight 1
#        inhibit_on_failure
#        MISC_CHECK {
#            misc_path "/etc/keepalived/check_realserver.sh ${O_P_DEST_IP_ARRAY[$_i]} ${O_P_SIDORPORT}"
...
EOF
      ...
      logger_info "program will just inserting virtual_server to an existing vrrp_instance:${O_P_VIP}"
      declare -i _flag
      _flag=$($AWK '
        {
          if($0 ~/^[[:space:]]*vrrp_instance[[:space:]]+vi_'"$O_P_DBTYPE"'_'"$O_P_VIRTUAL_ROUTER_ID"'[[:space:]]*{[[:space:]]*$/)
          {
            I=1;
            while (I>0) {
              getline
