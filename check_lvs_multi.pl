...
# 初始化log4perl
my $checkLog = "/var/log/check_lvs_$g_vrrpins.log";
my $logConf = q(
...
# 设置文件锁，防止脚本并行执行
my $lockFile="/tmp/check_lvs_$g_vrrpins.lock";
...
foreach my $line (@haconf_data){
    ...
    # 类型不为lvs，正常退出
    if ( lc($dbtype) ne "lvs" ) {
        $log->info("$g_host:$sidorport $dbtype is not lvs");
...
sub parallelCheck{
    ...
    # 获取mysql实例读vip
    my $rvip = `grep -v "#" $g_inscnf | egrep -w $sidorport | grep -wi $vrrpins | awk {'print \$9'}| uniq`;
    chomp($rvip);
    $log->debug("get read vip: $rvip");
